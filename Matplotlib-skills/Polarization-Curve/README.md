...
| **軸設定** | X / Y（左）/ Y₂（右，功率）三軸各自可設 min/max 範圍、刻度數量與**子刻度開關**（空=自動）|
| **×(−1) 切換** | 電流與電壓各自獨立「×(−1)」——數據負時翻正、正時翻負，使用者自行決定 |
| **單位顯示** | 主畫面下拉切換圖上電流單位標籤（A/cm² / mA/cm² / A）|
| **功率密度疊圖** | P = I×V 自動計算，雙 Y 軸顯示，單位可選 W/cm² / mW/cm² |
| **軸角色切換** | X=I,Y=V（電化學慣例）⇄ X=V,Y=I |
| **視覺微調** | 顏色、字型、**標題字體**與**刻度字體**獨立大小、線型/線寬、marker（種類/開關）|
| **功率單位** | 主畫面下拉：W/cm² / mW/cm² / W（功率曲線即時換算）|
...
3. 新增後可在「✎ 屬性」中進一步設定：
   ...
   - **×(−1)**：電流/電壓各自翻轉正負（數據負時翻正、正時翻負）
   - **功率曲線**：勾選顯示 + 單位（W/cm² / mW/cm²）

### 全域設定

- **軸角色**：`X=I, Y=V`（電化學慣例）或 `X=V, Y=I`
- **功率密度顯示**：勾選時所有勾了功率的曲線顯示在右 Y 軸（虛線）
- **單位顯示**：圖上電流單位標籤（A/cm² / mA/cm² / A）
- 字型 / 字體大小
